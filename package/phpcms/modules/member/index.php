...
class index extends foreground {
	...
	public function init() {
		//取得v9_member数据库数据，将值赋给$memberio
	...
	public function baoming(){
		...
		if(!$member_setting['allowregister']) {
			...
			$userinfo = array();
			$userinfo['nickname']=
			$userinfo['username'] = (isset($_POST['username']) && is_username($_POST['username'])) ? $_POST['username'] :"还没有成功！";
			$userinfo['phone'] = (isset($_POST['phone']) && is_username($_POST['phone'])) ? $_POST['phone'] : '';
			$userinfo['qq'] = (isset($_POST['qq']) && is_username($_POST['qq'])) ? $_POST['qq'] : "还没有成功！";
			$test= pc_base::load_model('member_test_model');
		
			$a=$test->insert($userinfo);
			
			//showmessage(L('operation_success'), 'index.php?m=member&c=index&a=init');
			
	
		
		include template('member','baoming');
	}
